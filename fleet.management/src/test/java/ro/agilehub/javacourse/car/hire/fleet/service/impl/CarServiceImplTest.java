...
public class CarServiceImplTest {
    ...
    public void findById_whenId_thenResult() {
        final String carId = "666f6f2d6261722d71757578";
        ...
        Car car = Car.builder().id(new ObjectId(carId)).mileage(mileage).model(carModel).build();
    ...
    public void findById_whenIdNotFound_thenException() {
        ...
        when(carRepository.findById(new ObjectId(carId))).thenReturn(Optional.empty());
    ...
    public void deleteById_whenId_thenResult() {
        ...
        doAnswer(invocationOnMock -> {
            ObjectId carIdToDelete = invocationOnMock.getArgument(0);
            assertEquals(new ObjectId(carId), carIdToDelete);
            return null;
    ...
    public void addCar_whenInput_thenResult() {
        CarDO carDO = CarDO.builder().carClass(CarDTO.CarClassEnum.CITY_CAR.getValue()).mileage(1111).fuel("Diesel")
                .registrationNumber("testRegistration").build();
        Car car = Car.builder().carClass(CarDTO.CarClassEnum.CITY_CAR.getValue()).mileage(1111).fuel("Diesel")
                .registrationNumber("testRegistration").build();
